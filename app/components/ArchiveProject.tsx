...
function getImageRatio(img: any): Ratio | null {
  ...
  // Sanity: пробуємо витягнути з asset.metadata.dimensions
...
export const ArchiveProject: React.FC<Props> = ({
  archiveProject,
  onPrev,
  onNext,
}) => {
  // те, що показуємо зараз
  ...
  // коли приходить новий проп — готуємо pending
  useEffect(() => {
  ...
  // аспект рахуємо по ACTIVE, щоб не було скачків
  const ratio = useMemo(
  ...
  // як тільки pending завантажився — свопимо
  const commitSwap = () => {
    setPendingImage((prevPending: any) => {
      ...
      requestAnimationFrame(() => {
        ...
        // невелика затримка, щоб закінчилася анімація
    ...
    <>
          ...
          {activeSrc && (
            <Image
              ...
              className={`object-contain transition-opacity duration-200 ${
                isPendingVisible || isFading ? 'opacity-0' : 'opacity-100'
              }`}
            />
          )}

          {/* PENDING */}
          {pendingImage && pendingSrc && (
            <Image
              ...
              placeholder="empty"
              className={`object-contain transition-opacity duration-200 ${
                isPendingVisible ? 'opacity-100' : 'opacity-0'
              } pointer-events-none`}
            />
          )}
        </div>

        <div className="mt-[12px] text-center">{archiveProject.title}</div>
      ...
      <section className="relative w-full min-h-screen items-center sm:flex hidden">
        <div className="px-[24px] grid grid-cols-24 w-full items-center content-center gap-y-[55px]">
          <div
            className={`relative flex items-center justify-center ${wrap} ${aspect}`}
          >
            {/* ACTIVE */}
            {activeSrc && (
              <Image
